...
import { ChevronLeft, ChevronRight } from "@/components/common/Icon/index";
...
function IconLeft({ className = "", ...props }: IconProps) {
  return <ChevronLeft className={cn("h-6 w-6", className)} {...props} />;
}

function IconRight({ className = "", ...props }: IconProps) {
  return <ChevronRight className={cn("h-6 w-6", className)} {...props} />;
}
...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      classNames={{
        // 월 컨테이너
        months: "flex flex-col lg:flex-row gap-2 w-full justify-center",
        ...
        head_row: "w-full flex items-center justify-between h-[32px] lg:h-10",
        head_cell: "text-gray-0 text-s w-10 text-center font-normal",
        row: "flex w-full mt-[6px] lg:mt-2 items-center justify-between",
        // 날짜 셀과 날짜
        cell: cn(
          "relative p-0",
        ...
        // 특수 상태의 날짜
        // 선택된 날짜
        day_selected: "bg-primary focus:bg-primary rounded-full text-white",
        // 오늘 날짜
